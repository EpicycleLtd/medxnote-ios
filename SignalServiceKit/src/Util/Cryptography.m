...
+ (nullable NSData *)decryptAESGCMWithData:(NSData *)encryptedData key:(OWSAES128Key *)key
{
    ...
    if (![decryptAuthTag ows_constantTimeIsEqualToData:encryptAuthTag]) {
        OWSFail(@"Auth tags don't match given tag: %@ computed tag: %@", encryptAuthTag, decryptAuthTag);
        return nil;
    }
